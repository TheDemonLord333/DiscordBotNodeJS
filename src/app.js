  ...
  searchTimeout = setTimeout(() => this.searchUsers(e.target.value), 500);
});
  ...
  dmsList.innerHTML = users.map(user => `
            <div class="dm-user" onclick="botController.selectUser('${user.id}', '${this.escapeHtml(user.username)}', '${this.escapeHtml(user.displayName)}')">
                <div class="user-avatar">
                    ${user.avatar ? `<img src="${user.avatar}" alt="${user.username}" style="width: 100%; height: 100%; border-radius: 50%;">` : user.username.charAt(0).toUpperCase()}
                </div>
                <div class="user-info">
